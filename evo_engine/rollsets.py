    ...
    while True:
        ...
        if point is None:
            if total in (4,5,6,8,9,10):
                ...
                # craps on come-out, new shooter continues
                continue
            elif total in (7,11):
                continue
        else:
...
def generate_roll_set(num_shooters: int, seed: int | None = None) -> dict:
    ...
    all_rolls = []
    for _ in range(max(1, int(num_shooters))):
        all_rolls.extend(_roll_until_seven_out(rng))
    return {"rolls": all_rolls, "meta": {"num_shooters": num_shooters, "seed": seed}}

def compute_table_cq(roll_set: dict) -> int:
    # Heuristic: score goes down with back-to-back PSOs and many 7s; up with longer points
    rolls = roll_set.get("rolls", [])
    if not rolls: return 60
    totals = [sum(r) for r in rolls]
    sevens = totals.count(7)
    points_made = sum(1 for i in range(1,len(totals)) if totals[i-1] in (4,5,6,8,9,10) and totals[i]==totals[i-1])
    # normalize
    length = max(1, len(totals))
    seven_rate = sevens/length
    made_rate = points_made/length
    cq = 70 + int(20*made_rate) - int(50*seven_rate)
    return max(0, min(100, cq))

def validate_table_cq(cq: int, ok_range=(40,90)) -> bool:
    return ok_range[0] <= cq <= ok_range[1]
