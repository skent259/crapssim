...
def test_event_order_and_types():
    r = client.post(
        "/step_roll", json={"session_id": "evt1", "mode": "inject", "dice": [2, 3]}
    )
...
def test_event_fields_required():
    r = client.post(
        "/step_roll", json={"session_id": "evt2", "mode": "inject", "dice": [4, 4]}
    )
    e = r.json()["events"][0]
    for key in [
        "type",
...
def test_roll_started_mode_reflects_request():
    r = client.post(
        "/step_roll", json={"session_id": "evt3", "mode": "inject", "dice": [1, 6]}
    )
    evs = r.json()["events"]
